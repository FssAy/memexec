...
pub struct DllLoader {
    pub entry_point_va: *const c_void,
...
impl DllLoader {
    pub unsafe fn new(
        ...
        if entry_point == 0 {
            ...
            #[cfg(feature = "hook")]
            let base = load_pe_into_mem(pe, hooks)?;

            #[cfg(not(feature = "hook"))]
            let base = load_pe_into_mem(pe)?;

            let idd = &pe.pe_header.nt_header.get_data_directory()[IMAGE_DIRECTORY_ENTRY_EXPORT];
            let ied = &*mem::transmute::<_, *const IMAGE_EXPORT_DIRECTORY>(
                base.offset(idd.VirtualAddress as isize),
            );

            let func_rvas = &*slice_from_raw_parts(
                base.offset(ied.AddressOfFunctions as isize) as *const u32,
                ied.NumberOfFunctions as usize
            );
            let name_rvas = &*slice_from_raw_parts(
                base.offset(ied.AddressOfNames as isize) as *const u32,
                ied.NumberOfNames as usize
            );
            let ordinals = &*slice_from_raw_parts(
                base.offset(ied.AddressOfNameOrdinals as isize) as *const u16,
                ied.NumberOfNames as usize
            );

            let mut exports = HashMap::new();

            for i in 0..ied.NumberOfNames as usize {
                let name = CStr::from_ptr(
                    base.offset((&*name_rvas)[i] as _) as *const _
                );
                ...
                let func_ptr = base.offset(func_rvas[ordinal] as _);
                exports.insert(
                    name.to_string(),
                    func_ptr as usize
                );
            ...
            #[cfg(feature = "hook")]
            let entry_point_va = base.offset(entry_point);
            #[cfg(not(feature = "hook"))]
            let entry_point_va = load_pe_into_mem(pe)?.offset(entry_point);
            Ok(DllLoader {
                entry_point_va,
